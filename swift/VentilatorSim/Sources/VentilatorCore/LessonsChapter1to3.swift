...
extension LessonLibrary {
    ...
    static let lesson1_1 = Lesson(
        ...
        points: ["波形は上から Paw / Flow / Volume",
                 ...
                 "設定の Vt と実測の Vte は別物"],
        tasks: [
            .talk(.scene, "ハルト君（8歳）が PICU のベッドに落ち着いた。虫垂炎の手術は終わったが、ショックが続くあいだは挿管のままだ。"),
            .talk(.doctor, "今夜の担当は{名前}先生です。まずは、この機械が何を言っているのかを読めるようにしましょう。"),
            .talk(.puku, "うわ、波形が 3 つもある…。どれが何なの？"),
            .talk(.doctor, "上から順に名前を言います。いちばん上は気道内圧（Paw）。気道の中の圧で、機械が空気を押し込むと高くなります。"),
            .talk(.puku, "圧の単位の cmH₂O って、なに？"),
            .talk(.doctor, "水を何 cm 押し上げられるか、で表した圧です。呼吸器で扱う圧はとても小さいので、この単位を使います。"),
            .talk(.doctor, "真ん中は流量（Flow）。空気が流れる速さと向きです。1 分あたり何 L の勢いか（L/分）で表します。"),
            .talk(.doctor, "いちばん下は換気量の波形（Volume）。その呼吸で肺に入った空気の量（mL）です。"),
            .talk(.doctor, "3 つとも呼吸に合わせて動きますが、1 つだけ動き方が違います。名前の意味を思い出しながら見てください。")
                .looking(["wave"]),
            .quiz("3 つのうち、ゼロ線をまたいで上にも下にも振れるのはどれですか。",
                  ["気道内圧",
                   ...
                   "換気量",
                   "二酸化炭素濃度"], answer: 1,
                  miss: ["気道内圧は、吐き終わっても残しておく圧（PEEP）より下がりません。ずっとゼロより上です。", nil,
            ...
            .talk(.doctor, "触るのは一回換気量（Vt）。1 回の呼吸で送り込む空気の量で、人が決める設定値です。"),
            ...
            .talk(.doctor, "次は Vte を見ます。吐いて戻ってきた空気を、機械が測った量です。e は呼気（expiratory）の e。"),
            .talk(.doctor, "Vt は「送って」と頼んだ量、Vte は実際に返ってきた量。この 2 つを見比べるのが、呼吸器を読む第一歩です。"),
            ...
            .talk(.doctor, "ほかの設定も、名前だけ先に。RR は 1 分間の呼吸回数、PEEP は吐き終わっても残しておく圧、FiO₂ は吸わせる酸素の濃さです。"),
            .talk(.puku, "入れた量と、戻ってくる量って、ちがうことあるの？")
                .looking(["val:Vte"]),
            .talk(.doctor, "あります。しかもそれが、いちばん最初に気づくべき異常です。"),
            .talk(.doctor, "子どもの気管チューブには、すき間をふさぐ風船（カフ）が付いていないものもあります。それを頭に置いて考えてください。"),
    ...
    static let lesson1_2 = Lesson(
        ...
        tasks: [
            .talk(.scene, "ハルト君の設定はひとまず落ち着いた。いぶき先生が、画面の圧の数字を指さす。")
                .looking(["val:PIP"]),
            .talk(.doctor, "画面の PIP は最高気道内圧。1 回の呼吸のあいだで、気道内圧がいちばん高くなった瞬間の値です。"),
            ...
            .talk(.doctor, "空気を送るには、細い管の中を押し流す圧と、肺をふくらませる圧が要ります。管の通りにくさを気道抵抗と言います。"),
            .talk(.doctor, "吸気ポーズは、吸い終わった瞬間に弁を閉じ、空気の流れを一瞬止める操作です。流れが止まれば、押し流す分の圧は消えます。"),
            .key("「吸気ポーズ」を押してください。",
            ...
            .talk(.doctor, "平らになったところが Pplat（プラトー圧）。流れが止まっても残っている、肺胞そのものにかかっている圧です。"),
            ...
            .talk(.doctor, "吸気流量は、吸気のときに機械が空気を送り込む速さ（L/分）。VC では、この速さも人が決める設定です。"),
    ...
    static let lesson1_3 = Lesson(
        ...
        tasks: [
            ...
            .talk(.doctor, "Cstat（静的コンプライアンス）は、圧を 1 cmH₂O かけたとき肺が何 mL ふくらむか。大きいほど柔らかい肺です。"),
            .talk(.doctor, "Raw（気道抵抗）は、空気を流すのにどれだけ圧が要るか。大きいほど通りにくい。痰やチューブの細さで大きくなります。"),
    ...
    static let lesson2_1 = Lesson(
        ...
        points: ["小児は実体重で決める（成人の予測体重式は使わない）",
                 ...
                 "肥満児だけは身長相当の標準体重で"],
        tasks: [
            .talk(.scene, "朝の回診。申し送りのとおり、ハルト君の一回換気量は 300 mL に上げられていた。"),
            .talk(.doctor, "ここからは自分で決めます。最初に決めるのは一回換気量。基準は体重です。"),
            .talk(.puku, "体重？ 身長じゃなくて？"),
            .talk(.doctor, "小児では体重 1 kg あたり 6〜8 mL。体重はカルテにあります。"),
            .talk(.doctor, "入れすぎると肺胞が伸ばされすぎて傷みます（人工呼吸器による肺傷害）。これを避けることを「肺保護」と言います。"),
            .key("「患者情報」キーを押して、ハルト君の体重を確かめてください。",
                ...
                .spotting(["hard:kPt"]),
            .talk(.puku, "mL/kg って、どうやって出すの？"),
            .talk(.doctor, "Vte を体重で割るだけです。体重 1 kg あたり何 mL 入っているか、という意味になります。"),
    ...
    static let lesson2_2 = Lesson(
        ...
        tasks: [
            ...
            .talk(.puku, "ぶんじかんきりょう？"),
            .talk(.doctor, "分時換気量（MV）は、1 分間に肺を出入りする空気の総量。一回換気量 × 呼吸回数（RR）で出ます。"),
            .talk(.doctor, "体の CO₂ は、吐く息でしか外に出られません。だから MV が増えれば CO₂ は下がり、減れば上がります。"),
            ...
            .talk(.doctor, "1 回の呼吸は、吸う時間（吸気時間）と吐く時間（呼気時間）の 2 つでできています。吸気時間は設定でほぼ決まっています。"),
    ...
    static let lesson2_3 = Lesson(
        ...
        tasks: [
            ...
            .talk(.doctor, "FiO₂ は吸わせる酸素の濃さ。部屋の空気は 21%、最大は 100% です。濃い酸素を長く吸わせると、それ自体が肺を傷めます。"),
            .talk(.doctor, "SpO₂ は指先のセンサーで測る、血液が運んでいる酸素の割合です。この年齢の子の目標は 94〜98%。"),
            ...
            .talk(.doctor, "PEEP は、吐き終わっても気道に残しておく圧。肺胞がしぼみきって潰れないように、内側から支えます。"),
            .step("PEEP を 12 cmH₂O まで上げて確定してください。",
                  check: { $0.settings.peep >= 12 })
                .spotting(["key:peep"]).watching(["ABP mean", "Pplat", "SpO₂"]),
            .talk(.doctor, "平均動脈圧（ABP mean）は、1 拍を通した血圧の平均。臓器に血液が届いているかの目安です。"),
    ...
    static let lesson3_1 = Lesson(
        ...
        tasks: [
            .talk(.scene, "3 番ベッドのミオちゃん（3歳）。インフルエンザ肺炎で、胸の X 線は両肺が真っ白だ。"),
            .talk(.doctor, "同じ肺でも、量を決めるか圧を決めるかで、この子の安全域が変わります。"),
            .talk(.doctor, "ミオちゃんは小児 ARDS。肺全体に炎症が起き、肺胞が水びたしになって潰れ、肺が硬くなっている状態です。"),
            ...
            .talk(.doctor, "VC（量規定）は、毎回決まった量を送るやり方。PC（圧規定）は、決まった圧までふくらませ、その圧を保つやり方です。"),
            .talk(.doctor, "後ろの AC は、すべての呼吸を機械が同じ形で送るという意味。いまの VC-AC を PC-AC に変えます。"),
            .key("モードを「PC-AC」に切り替えてください。", event: .modePressureAC,
                 why: "次の呼吸から、圧波形が四角い台形に変わります。設定した圧まで一気に上げ、吸気のあいだ保つからです。")
                .spotting(["mode:A/C PC"]),
            .talk(.doctor, "P insp は、PC で決める吸気の圧。PEEP にどれだけ上乗せするかの値です。上げるほど、入る量が増えます。"),
    ...
    static let lesson3_2 = Lesson(
        ...
        points: ["トリガ感度は「吸ったと見なす」しきい値。小児ほど敏感に",
                 ...
                 "敏感すぎ・リーク＝オートトリガ"],
        tasks: [
            .talk(.scene, "昇圧薬が減り、ハルト君の鎮静を浅くしはじめた。胸が、機械より先に動きはじめている。"),
            .talk(.doctor, "ここから患者が呼吸に参加します。その「吸いたい」を機械がどう受け取るかが、トリガです。"),
            .talk(.doctor, "トリガは「引き金」のこと。患者が吸おうとした瞬間を機械が見つけて、送気を始める仕組みです。"),
            ...
            .talk(.doctor, "RR tot は実際の呼吸回数。設定した回数に、この子が自分で吸った回数が上乗せされます。"),
            ...
            .talk(.doctor, "トリガ感度は、回路にこれだけの流れ（L/分）が生まれたら「吸った」と見なす、というしきい値。数字が大きいほど鈍くなります。"),
    ...
    static let lesson3_3 = Lesson(
        ...
        tasks: [
            ...
            .quiz("SIMV で設定 RR 10、実測 RR 18 でした。差の 8 回は何ですか。",
                ...
                .looking(["val:RR tot"]),
            .talk(.puku, "SIMV と PSV、聞いただけだと、まだピンとこない…"),
            .talk(.doctor, "SIMV は決めた回数だけ必ず送ります。PSV は機械からは 1 回も送りません。切り替えれば分かります。"),
